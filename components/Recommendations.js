  ...
  const handleCareerSelect = async (career) => {
    try {
      if (!selectedCareers.includes(career.title)) {
        ...
        // Update database with selected careers
        if (session?.user?.email) {
          console.log("Updating selected careers in database:", newSelectedCareers);
          const result = await updateSelectedCareers(session.user.email, newSelectedCareers);

          if (result.error) {
            console.error("Error saving career selection:", result.error);
          } else {
            console.log("Career selection saved successfully:", result);
          }
        }
